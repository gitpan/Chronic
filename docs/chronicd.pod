...
schedule high-impact, recurring tasks like C<updatedb>, C<emerge
rsync>, C<webalizer> when I was away from the computer. This
...
Chronic is implemented as a Unix daemon, C<chronicd>, that reads
a C<chrontab> file and executes tasks specified therein. A
system level C<chrontab> is placed in C</etc/chrontab> and
user-specific C<chrontab>'s live in C<$HOME/.chrontab>. The only
option C<chronicd> accepts at the moment is C<-fg> that
instructs the application to run in foreground and print
debugging messages.

=head1 CHRONTAB FORMAT

C<chrontab> contains a task per line. A task consists of a
C<command> to be run, and one or more C<constraint>'s that must
be met before the command is run. It should be noted that as of
the current version, when more than one C<constraint> is
provided for a task, they are C<AND>'ed together. The format of
...
The C<Inactivity> constraint monitors system activity and is met
...
following task will run C<updatedb> when the system has been
inactive for 120 seconds.

    command = "/usr/bin/updatedb"; constraint = Inactivity, 120;

C<Inactivity> is a wrapper around the DiskIO and Loadavg
...
The C<Freq> constraint specifies the maximum frequency of
invocation of a task. C<Freq> requires one parameter, the number
of seconds, that is the minimum difference between two
consecutive invocations. So, C<Freq, 86400> implies that the 
task should not be run more than once a day.  C<Freq> is usually 
coupled with another constraint (eg: C<Inactivity>) to
...
The C<Loadavg> constraint monitors the load average of the
...
The syntax is C<Loadavg, TIME, LOADAVG>. eg. C<Loadavg, 60, 0.02> 
...
C</home/user/project/> and C<make> if the load average is 0.0
for 60 seconds. The C<Freq> constraint additionally limits the
task to no more than once in 3600 seconds (one hour).

=item The C<DiskIO> Constraint

The C<DiskIO> constraint monitors the disk read/write activity
through the C<vmstat> program. It accepts three parameters: a
threshold for number of blocks read from disk, a threshold for
number of blocks written to disk and the amount of time for
which read/write linger at or below these thresholds. C<DiskIO>
syntax is C<DiskIO, TIME, BLOCKS WRITTEN, BLOCKS
READ>. eg. C<DiskIO, 60, 10, 15> is met when 10 or less blocks
...
The C<Xscreensaver> constraints asynchronously monitors
Xscreensaver through the C<xscreensaver-command> program. It
accepts one parameter: the number of seconds for which
C<Xscreensaver> must be enabled before the constraint is met.
The syntax for the constraint is C<Xscreensaver, TIME>. eg.
C<Xscreensaver, 120> is met when C<Xscreensaver> is locked for
...
=item The C<InXs> Constraint

The C<InXs> constraint (unrelated to the POP band) combines the
Xscreensaver and Inactivity constraints. It is met when
B<either> of the constraints is met. C<InXs> is useful for
...
of the constraints is enabled before C<InXs> is met. The syntax
for the constraint is C<InXs, TIME>. eg. C<InXs, 120>. Here's an
example of usage:
...
In addition to C<command> and C<constraint> keys, a chrontab
...
C<notify> is used to specify a recipient for an email
notification that is sent with the task is executed. This
notification contains information on timing and status of the
executed task. Syntax C<notify = admin@domain>.

=item only_once

C<only_once> is used to indicate that the task should be
...
C<http://sourceforge.net/projects/chronic> or send mail to
the author.
