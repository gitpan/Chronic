...
System Inactivity, Disk I/O, Load Average, Frequency and Xscreensaver
constraints. Network IO activity, cron (that emulates cron), and file
alterations under a specified directory, are some of the constraints
...
command is run. It should be noted that as of the current version, when
more than one C<constraint> is provided for a task, they are C<AND>'ed
together. The format of task specification is:
...
Xscreensaver and Inactivity constraints. It is met when B<either> of the
constraints is met. C<InXs> is useful for scheduling high-load
applications on end-user machines because it's a good indicator of the
user being away from the computer. It accepts one parameter: the number of
seconds for which either of the constraints is enabled before C<InXs> is
met. The syntax for the constraint is C<InXs, TIME>. eg. C<InXs, 120>.
